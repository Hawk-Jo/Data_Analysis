...
## 프로젝트 개요
- 현황: 주간 리포트에서 결제일 간격 기반 이번 주 결제일 예측 서비스를 제공
- 문제: 고객으로부터 예측 정확도가 너무 낮다는 피드백을 받음
- 목표: 이번 주 결제일 예측 정확도 향상
- 방법
    1. EDA를 통해 많은 SaaS들이 특정 날짜를 기준으로 트랜잭션이 발견하는 것을 확인
    2. 결제일 빈도 기반 예측 로직 구성 및 검증
- 결과
    1. 결제일 빈도 기반 예측이 결제일 간격 기반 예측보다 정밀도, 재현율, F1 점수 모두 10배 이상 높음
    2. 실제 서비스 대입 후 정확도가 향상되었다는 피드백을 받음
